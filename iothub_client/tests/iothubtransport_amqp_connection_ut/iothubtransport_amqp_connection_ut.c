...
TEST_FUNCTION(amqp_connection_create_SASL_and_CBS_negative_checks)
{
	...
	for (size_t i = 0; i < umock_c_negative_tests_call_count(); i++)
	{
		...
		if (i == 10 || // connection_set_trace
			i == 11 || // free(unique_container_id)
			i == 12 || // STRING_c_str(instance->iothub_fqdn) for connection_create2
			i == 14 || // session_set_incoming_window
			i == 15)   // session_set_outgoing_window
		{
			continue; // these lines have functions that do not return anything (void) or do not cause failures.
		}
		else if (i == 9) // connection_create2
		{
